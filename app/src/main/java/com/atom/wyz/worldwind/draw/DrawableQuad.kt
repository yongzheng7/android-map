...
class DrawableQuad : Drawable {

    companion object {
        fun obtain(pool: Pool<DrawableQuad>): DrawableQuad {
            return pool.acquire()?.setPool(pool) ?: DrawableQuad().setPool(pool) // get an instance from the pool
        }
    }
    ...
    private var pool: Pool<DrawableQuad>? = null

    private fun setPool(pool: Pool<DrawableQuad>): DrawableQuad {
        this.pool = pool
        return this
    }
    ...
    override fun draw(dc: DrawContext) {
        ...
        this.program!!.loadModelviewProjection(mvpMatrix)

        GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, 4)

        var next: Drawable?
        while (dc.peekDrawable().also { next = it } != null && next?.let { canBatchWith(it) } ?: false) {
            val drawable = dc.pollDrawable() as DrawableQuad?
    ...
    protected fun canBatchWith(that: Drawable): Boolean {
        return (this.javaClass == that::javaClass && program === (that as DrawableQuad).program && color.equals((that as DrawableQuad).color)
